 ...
 */
public class CameraPushMainView extends RelativeLayout implements ITXLivePushListener,
    ...
    public CameraPushMainView(Context context) {
      super(context);
      initView(context);
    }

    public CameraPushMainView(Context context, AttributeSet attrs) {
      super(context, attrs);
      initView(context);
    }

    public CameraPushMainView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//      mWindowManager.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
//      setTheme(R.style.LivePusherBeautyTheme);
//      setContentView(R.layout.livepusher_activity_live_pusher_main);
      mRootView = (ViewGroup) LayoutInflater.from(context).inflate(R.layout.livepusher_activity_live_pusher_main, null);
      addView(mRootView);
      initData();                // 初始化数据
...
//      startPush();
    ...
    public void onPushEvent(int event, Bundle param) {
        ...
        String pushEventLog = mContext.getString(R.string.livepusher_receive_event) + event + ", " + msg;
...
//        mRealtimePlayURL = intent.getStringExtra(Constants.INTENT_URL_PLAY_ACC);
    ...
    private void initFragment() {
        if (mPusherSettingFragment == null) {
            ...
            mPusherSettingFragment.loadConfig(mContext);
        ...
        if (mPusherVideoQualityFragment == null) {
            ...
            mPusherVideoQualityFragment.loadConfig(mContext);
...
//        });
    ...
    private Uri getUri(File file) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            uri = FileProvider.getUriForFile(mContext, "com.tencent.liteav.demo", file);
        } else {
...
//                mBtnStartPush.setBackgroundResource(R.drawable.livepusher_pause);
            ...
            case Constants.PLAY_STATUS_INVALID_URL:
                ...
                params.putString(TXLiveConstants.EVT_DESCRIPTION, mContext.getString(R.string.livepusher_check_url));
                mLogInfoWindow.setLogText(null, params, LogInfoWindow.CHECK_RTMP_URL_FAIL);
                break;
            case Constants.PLAY_STATUS_LICENSE_ERROR:
                String errInfo = mContext.getString(R.string.livepusher_license_check_fail);
                int start = (errInfo + mContext.getString(R.string.livepusher_license_click_info)).length();
                int end = (errInfo + mContext.getString(R.string.livepusher_license_click_use_info)).length();
                SpannableStringBuilder spannableStrBuidler = new SpannableStringBuilder(errInfo + mContext.getString(R.string.livepusher_license_click_use_info));
                ...
                dialogBuilder.setTitle(mContext.getString(R.string.livepusher_push_fail)).setView(tv).setPositiveButton(mContext.getString(R.string.livepusher_comfirm), new DialogInterface.OnClickListener() {
                    @Override
        ...
        if (code != Constants.PLAY_STATUS_INVALID_URL) {
            ...
            bundle.putString(TXLiveConstants.EVT_DESCRIPTION, mContext.getString(R.string.livepusher_check_url));
...
//        mAudioEffectPanel.resumeBGM();
    ...
    private void setRotationForActivity() {
        ...
        WindowManager mWindowManager = (WindowManager) mContext.getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
        int mobileRotation = mWindowManager.getDefaultDisplay().getRotation();
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (file.exists() && file.length() > 0) {
                  ...
                  mContext.startActivity(Intent.createChooser(intent, mContext.getString(R.string.livepusher_share_pic)));
                } else {
                    showToast(R.string.livepusher_screenshot_fail);
                }
            }
